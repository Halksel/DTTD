...
bool Object::InMouseDoubleClick() {
	return x <= getMouseX() && (getMouseX() <= x + width*expWRate) && y <= getMouseY() && (getMouseY() <= y + height*expHRate) && Input.GetMousePressDouble(MyInput::MOUSE_1);
}

bool Object::InMousePointer(){
	return x <= getMouseX() && (getMouseX() <= x + width*expWRate) && y <= getMouseY() && (getMouseY() <= y + height*expHRate);
}

shared_ptr<Object> Object::GetPtr() {
